...
func (d *Deej) initializeTray(onDone func()) {
	...
	onReady := func() {
		...
		refreshCon := systray.AddMenuItem("Re-connect", "Manually refresh connect COM port")
		refreshCon.SetIcon(icon.RefreshSessions)
		...
		go func() {
			for {
				...
				case <-refreshCon.ClickedCh:
					logger.Info("Reconnecting the Sound Mixer")
					d.serial.Stop()
					time.Sleep(3 * time.Second)
					if err := d.serial.Start(); err != nil {
						d.notifier.Notify(fmt.Sprintf("Failed to connect %s!", d.config.ConnectionInfo.COMPort),
							"This serial port doesn't exist, check your configuration and make sure it's set correctly.")
						logger.Warnw("Failed to connect", "error", err)
